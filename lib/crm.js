...
// ⬇️ MODIFIED: This function is updated to handle the 'environment' parameter.
async function getEvents(options = {}) {
  const token = await getAccessToken();
  const { entityids, environment = 'Production' } = options;
...
// ⬇️ MODIFIED: This function is updated to handle the 'environment' parameter.
async function getEventPriceLevel(eventId, environment = 'Production') {
  if (!eventId) {
...
(async () => {
  if (require.main === module) {
    ...
    try {
      ...
      // ⬇️ MODIFIED: Updated to use the new function signature for testing.
      const allEventsResponse = await getEvents({ environment: 'Production' });
      ...
      if (allEventsResponse && allEventsResponse.value && allEventsResponse.value.length > 0) {
        ...
        // ⬇️ MODIFIED: Updated to use the new function signature for testing.
        const priceLevelResponse = await getEventPriceLevel(testEventId, 'Production');
